...
class GrpoTrainerTest(unittest.TestCase):
  ...
  def test_grpo_trainer_correctness(self):
    ...
    maxtext_model, state, reference_params, rng, state_mesh_shardings, data_sharding = setup_maxtext_model(self.config, self.mesh)
    ...
    for i, d in enumerate(engine_data[self.config.train_data_columns]):
      input_data.append(InputData(id=f"input_{i}", tokens=np.array(d), true_length=np.array(data[f"{self.config.train_data_columns}_true_length"][i])[0]))
    
    results = self.inference_engine.batch_inference(input_data)
    
    # Assert that the generated completions match the golden reference.
    self.assertEqual(results["prompt_completions"][0].tolist(), golden_data["generated_completions"])
