...
""" Tests for Verifying HLO Identify from Ahead of Time Compilation"""
...
class AotHloIdenticalTest(unittest.TestCase):
  ...
  def get_device_user_facing_name(self):
    ...
    topology_devices = num_devices
    # For v4 and v5p, the topology value is 2x the number of devices (chips)
    if "TPU v4" in device_kind:
      prefix = "v4"
      topology_devices = 2 * num_devices
    elif "TPU v5 lite" in device_kind:
      prefix = "v5e"
    elif "TPU v5p" in device_kind:
      prefix = "v5p"
      topology_devices = 2 * num_devices
    elif "TPU v6" in device_kind:
      prefix = "v6e"
    else:
